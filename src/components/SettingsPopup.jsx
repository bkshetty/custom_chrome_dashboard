...
const SettingsPopup = ({ setBackground, isOpen, setIsOpen }) => {
    ...
    return (
        <>
            {/* Toggle Button (Fixed securely to the viewport) */}
            <div className={`fixed bottom-4 right-4 z-[60] transition-opacity duration-300 ${isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100 pointer-events-auto'}`}>
                <button
                    onClick={() => {
                        setIsOpen(!isOpen);
                        setShowBgMenu(false); // Reset submenu when closing
                    }}
                    className="w-10 h-10 flex items-center justify-center cursor-pointer bg-black/40 hover:bg-black/60 backdrop-blur-md rounded-full transition-all duration-300 pointer-events-auto shadow-xl"
                    title="Customize Chrome"
                >
                    <Settings className="w-4 h-4 text-white/80 hover:text-white transition-colors" />
                </button>
            </div>

            <div className={`fixed top-0 right-0 h-full flex z-50 pointer-events-none transition-all duration-500 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                {/* Settings Menu Panel */}
                <div
                    className={`w-[340px] h-full liquid-glass rounded-none border-y-0 border-r-0 p-4 flex flex-col gap-2 shadow-2xl pointer-events-auto relative`}
                >
                    ...
                    <div className="text-white/90 font-medium px-2 py-1 mb-1 mt-2 text-xl">Customize Chrome</div>
